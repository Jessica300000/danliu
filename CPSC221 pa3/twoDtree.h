...
private:
   ...
   bool withinTolerance(Node *node, HSLAPixel avg, double tol);
   void pruneTree(Node *root, double tol);
   void renderPicture(PNG &pic, Node *&node);
   void clearChild(Node *&node);
   void copyTree(Node *&currRoot, Node *newRoot);
